...
function PageLoad() {
	$('body').removeClass('hide_this');
		...
		tl_loader.staggerFrom('.texts_loader path', 1, {delay:.3,autoAlpha:0,ease:Power2.easeOut},0.1);
		TweenMax.from('.figure_first', .6, {delay:1.5,x:-20,scale:.6,autoAlpha:0,ease:Power1.easeOut});
		TweenMax.from('.figure_second', .6, {delay:1.5,x:70,scale:.6,autoAlpha:0,ease:Power1.easeOut});

	setTimeout(function () {


			var tl = new TimelineLite();
			tl.staggerFrom('.title_home_eff .word', 1, {delay:1.4,x:20,autoAlpha:0,ease:Power2.easeOut},0.15);
			TweenMax.from('.subtitle_hom_g', 1, {delay:1.9,y:-20,autoAlpha:0,ease:Power2.easeOut});
			TweenMax.from('.abs_hom_sc', 1, {delay:2.3,y:-20,autoAlpha:0,ease:Power2.easeOut});
			TweenMax.from('.text_effect_go', 5, {delay:2.3,autoAlpha:0,ease:Power2.easeOut});

			$('body').removeClass('hidden_ball');


		TweenMax.to($(".loader_go"),0.7, {force3D:true, yPercent: -101, delay:1, ease:Power2.easeInOut});
		TweenMax.set($(".loader_go"), {visibility:'hidden', delay:1.8, opacity:0});
...
function animationsOnScroll() {
	...
	$('.has_opacity').each(function(){
		...
		scene.on('enter', function(){
			$this.delay($this.attr('data-delay')).queue(function(next){
				TweenMax.to($this, 0.8, {force3D:true, autoAlpha:1, delay:0.1, ease:Power2.easeOut});
				next();
			});
		});
	...
	$('.has_animation_from_right').each(function(){
		...
		scene.on('enter', function(){
			$this.delay($this.attr('data-delay')).queue(function(next){
				TweenMax.to($this, 0.8, {force3D:true, opacity:1, x:0, delay:0.1, ease:Power2.easeOut});
				next();
			});
		});
		scene.on('leave', function(event){
	...
	$('.has_mask').each(function(){
		...
		scene.on('enter', function(){
			...
			$this.find('.word').each(function(index, element) {
				tl.to(element, 0.6, {y:0, opacity:1, autoAlpha:1, delay:0.1, ease:Power2.easeOut}, index * 0.03)
			});
		});
		scene.on('enter', function() {
			$this.addClass('active');
	...
	$('.has_mask_right').each(function(){
		...
		scene.on('enter', function(){
			var tl = new TimelineLite();
			$this.find('.word').each(function(index, element) {
				tl.to(element, 0.6, {x:0, opacity:1, autoAlpha:1, delay:0.1, ease:Power2.easeOut}, index * 0.01)
			});
		});
		scene.on('enter', function() {
			$this.addClass('active');
...
function otherCode() {
	...
	$('.burger_go').on("click", function () {
		...
		if($(this).hasClass('active')) {
			...
			var tl = new TimelineLite();
			tl.staggerFrom('.menu_mobile_go .menu_head_go li,.languages_go', 1, {y:20,autoAlpha:0,ease:Power2.easeOut},0.1);
			hideScroll();
		}else {
